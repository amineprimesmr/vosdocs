...
function getBlogConfig() {
  ...
  if (!fs.existsSync(configPath)) {
    return {
      baseUrl: process.env.BASE_URL || 'https://www.vosdocs.fr',
      blogPath: '/blog',
      categories: {}
    };
  }
...
/**
 * Génère 2 articles SEO via Groq (thème : certificat de non-gage, carte grise, vente/achat véhicule)
 */
async function generateTwoArticles(existingSlugs = []) {
  ...
  const systemPrompt = `Tu es un rédacteur SEO expert pour un site français (VosDocs) qui vend des certificats de non-gage (situation administrative véhicule). 
Tu écris des articles utiles, précis et optimisés pour le référencement, sur les thèmes : certificat de non-gage, carte grise, vente et achat de véhicule d'occasion, démarches administratives.
Chaque article doit faire environ 800 à 1200 mots, en français, avec des titres H2 et du contenu structuré.
Le bodyHtml doit être du HTML valide (paragraphes <p>, listes <ul><li>, titres <h2>, liens <a href="https://www.vosdocs.fr/">VosDocs</a> quand pertinent).
Catégories possibles : ${CATEGORIES_LIST.join(', ')}.`;

  const userPrompt = `Génère exactement 2 articles de blog SEO différents pour VosDocs.
${slugsHint}
Pour chaque article, fournis un objet JSON avec les clés exactes :
- title (string, titre accrocheur)
- slug (string, URL-friendly, minuscules, tirets, pas d'accents, ex: "validite-certificat-non-gage")
...
async function createProposalsAndSendEmail(articles) {
  ...
  const baseUrl = process.env.BASE_URL || getBlogConfig().baseUrl || 'https://www.vosdocs.fr';
  const batchId = Date.now().toString();
  const merchantEmail = process.env.MERCHANT_EMAIL || process.env.MAIL_TO || 'infos.vosdocs@gmail.com';
...
<body style="font-family:system-ui,sans-serif;max-width:600px;margin:0 auto;padding:20px;color:#333;">
  <h1 style="font-size:1.25rem;">VosDocs – Nouvelles propositions d'articles</h1>
  ...
  <p style="color:#888;font-size:0.85rem;">Cet email est envoyé automatiquement par le système Auto-Blog VosDocs.</p>
</body>
</html>`;
  ...
  const { data, error } = await resend.emails.send({
    from,
    to: merchantEmail,
    subject: 'VosDocs – 2 propositions d’articles à valider',
    html
  });
