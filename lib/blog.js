...
async function callGroq(messages, options = {}) {
  ...
  for (let i = 0; i < candidates.length; i++) {
    ...
    if (!res.ok) {
      ...
      const canRetry = i < candidates.length - 1 && isMissingGroqModelError(res.status, err);
      if (canRetry) {
    ...
    if (!content) {
      ...
      if (i < candidates.length - 1) {
        console.warn('Réponse Groq vide, essai modèle suivant:', model);
        continue;
      }
      throw lastErr;
    }
    if (i > 0) console.warn('Groq: bascule sur', model);
    return content;
  }
...
async function generateTwoArticles(existingSlugs = []) {
  ...
  const systemPrompt = `Tu es un rédacteur SEO senior pour un site français (${brand}) spécialisé dans les démarches véhicule (rapport VIN / historique, carte grise, vente/achat d'occasion).
...
N'inclus jamais de montant en euros ni de prix fixe (ex. « 19,90 € ») dans le bodyHtml : les offres sont sur le site ; renvoie vers ${siteUrl} sans chiffrer un tarif.`;

  const userPrompt = `Génère exactement 2 articles de blog SEO différents pour ${brand}.
${slugsHint}
