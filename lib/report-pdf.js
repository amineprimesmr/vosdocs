...
function renderVehicleDatabasesBundle(doc, bundle, vin) {
  ...
  function emitSection(titleFr, renderFn) {
    n += 1;
    needPage(doc, 40);
    sectionTitle(doc, String(n), titleFr, null);
    renderFn();
  }
  ...
  if (vdPdfIncludeSection('stolenCheck', b)) {
    emitSection('Vérification antivol', () => renderStolenCheckVd(doc, b.stolenCheck));
  }
  if (vdPdfIncludeSection('marketValue', b)) {
    emitSection('Valeur marché (cote)', () => renderMarketValueVd(doc, b.marketValue));
  }
  if (vdPdfIncludeSection('recalls', b)) {
    emitSection('Rappels de sécurité', () => renderRecallsVd(doc, b.recalls));
  }
  if (vdPdfIncludeSection('salesHistory', b)) {
    emitSection('Historique des ventes', () => renderSalesHistoryVd(doc, b.salesHistory));
  }
  if (vdPdfIncludeSection('auction', b)) {
    emitSection('Enchères (Copart, IAAI, Manheim…)', () => renderAuctionVd(doc, b.auction));
  }
