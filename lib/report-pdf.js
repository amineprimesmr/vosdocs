...
function renderStolenCheckVd(doc, block) {
  ...
  const possible = records.some((r) => r.possible_stolen === true || String(r.possible_stolen).toLowerCase() === 'true');

  needPage(doc, 60);
  coloredBadge(
    doc,
    possible ? '⚠ VÉHICULE POSSIBLEMENT SIGNALÉ COMME VOLÉ' : '✓ Aucun signalement de vol',
    possible ? COL.errBg : COL.okBg,
    possible ? COL.redText : COL.greenText
  );

  if (possible) {
    subMuted(doc, 'Ce véhicule apparaît dans une base de signalement de vol. Vérifiez impérativement auprès de la gendarmerie ou de la police avant toute acquisition.');
  } else {
    subMuted(doc, 'Aucun vol signalé dans les bases consultées. Ne remplace pas une vérification officielle auprès des autorités compétentes.');
  }

  if (records.length > 0) {
    doc.font('Helvetica-Bold').fontSize(9).fillColor(COL.ink);
    doc.text('Détail des signalements :', 50, doc.y);
    doc.moveDown(0.3);
    records.forEach((r, i) => {
      needPage(doc, 50);
      doc.font('Helvetica-Bold').fontSize(9).fillColor(COL.ink);
      doc.text('Signalement ' + (i + 1), 50, doc.y);
      doc.moveDown(0.15);
...
function renderMarketValueVd(doc, block) {
  ...
  // Structure réelle VehicleDatabases :
  // market_value.market_value_data = [{ trim, "market value": [{ Condition, "Trade-In", "Private Party", "Dealer Retail" }] }]
  const marketValueData = d.market_value && Array.isArray(d.market_value.market_value_data)
    ? d.market_value.market_value_data
    : [];

  subMuted(doc, 'Estimation de valeur par condition : Outstanding (excellent) · Clean (bon) · Average (moyen) · Rough (mauvais état). Données marché US/Canada — prix en USD, indicatif.');

  if (basic.make || basic.model || basic.year) {
    rowKV(doc, 'Véhicule', [basic.year, basic.make, basic.model, basic.trim].filter(Boolean).join(' '));
  ...
  if (marketValueData.length === 0) {
    subMuted(doc, 'Cote non disponible dans les données retournées pour ce VIN.');
    return;
  }
  ...
  marketValueData.forEach((trimEntry) => {
    ...
    const condRows = Array.isArray(trimEntry['market value']) ? trimEntry['market value'] : [];
    ...
    condRows.forEach((row) => {
      ...
      // Les prix sont des strings comme "$5,392" — on les affiche directement
...
function renderRecallsVd(doc, block) {
  ...
  const d = block.data || {};
  const recalls = Array.isArray(d.data && d.data.recall ? d.data.recall : d.recall) ? (d.data && d.data.recall ? d.data.recall : d.recall) : [];
...
function renderSalesHistoryVd(doc, block) {
  ...
  const d = (block.data && block.data.data) ? block.data.data : (block.data || {});
  const history = Array.isArray(d.sales_history) ? d.sales_history : [];
  ...
  subMuted(doc, `${history.length} entrée(s) d\'historique. Permet de retracer les passages chez des concessionnaires / enchères.`);

  // Identité véhicule depuis les données sales
  ...
  history.slice(0, 10).forEach((sale, i) => {
    ...
    doc.text(`Vente ${i + 1}${sale.sale_status ? ' · ' + sale.sale_status : ''}`, 58, bTop + 3, { width: doc.page.width - 116 });
    doc.restore();
    doc.y = bTop + 20;
    doc.x = 50;

    if (sale.post_date) rowKV(doc, 'Date publication', formatDate(sale.post_date));
    if (sale.sale_date) rowKV(doc, 'Date vente', formatDate(sale.sale_date));
    ...
    if (lp) {
      ...
      if (typeof price === 'number' || (typeof price === 'string' && price !== '')) {
        rowKV(doc, 'Prix annoncé', formatMoney(price, cur));
      }
      if (lp.retail_value) rowKV(doc, 'Valeur de revente', formatMoney(lp.retail_value, cur));
      if (lp.repair_cost) rowKV(doc, 'Coût réparations estimé', formatMoney(lp.repair_cost, cur));
    }

    const imgs = Array.isArray(sale.images) ? sale.images : [];
    if (imgs.length > 0) {
      doc.font('Helvetica').fontSize(8).fillColor(COL.muted);
      doc.text(`${imgs.length} photo(s) disponible(s) pour cette vente.`, 50, doc.y, { width: doc.page.width - 100 });
      doc.moveDown(0.2);
    }
...
function renderAuctionVd(doc, block) {
  ...
  subMuted(doc, `${records.length} lot(s) d\'enchères. Indique les passages en vente aux enchères (Copart, IAAI, Manheim, etc.).`);

  records.slice(0, 8).forEach((r, i) => {
    needPage(doc, 90);
    const title = r.vname || ([r.year, r.make, r.model].filter(Boolean).join(' ')) || ('Lot ' + (i + 1));
    doc.font('Helvetica-Bold').fontSize(10).fillColor(COL.ink);
    ...
    if (r.price != null && r.price !== '') rowKV(doc, 'Prix adjugé', formatMoney(r.price, 'USD'));
    ...
    if (ts['Repair Cost']) rowKV(doc, 'Coût réparations', formatMoney(ts['Repair Cost'], 'USD'));

    const fKeys = Object.keys(cf || {});
    if (fKeys.length > 0) {
      const fStr = fKeys.map((k) => `${k}: ${cf[k]}`).join(' · ');
      rowKV(doc, 'Caractéristiques', fStr);
    }
...
function renderMediaVd(doc, block) {
  ...
  const colors = Array.isArray(images.colors) ? images.colors : [];
  const total = exterior.length + interior.length + colors.length;

  if (total === 0) {
    subMuted(doc, 'Aucune photo disponible pour ce véhicule dans les sources consultées.');
    return;
  }

  subMuted(doc, `${total} photo(s) disponible(s) — ${exterior.length} extérieur, ${interior.length} intérieur, ${colors.length} variante(s) de couleur.`);
  ...
  const renderPhotoList = (label, list) => {
    ...
    doc.moveDown(0.2);
    list.slice(0, 6).forEach((url) => {
      if (!url || typeof url !== 'string') return;
      needPage(doc, 16);
      doc.font('Helvetica').fontSize(7.2).fillColor('#2563eb');
      doc.text(url, 50, doc.y, { width: doc.page.width - 100, link: url, underline: true });
      doc.moveDown(0.15);
      doc.fillColor(COL.ink);
    });
    if (list.length > 6) subMuted(doc, `… et ${list.length - 6} autre(s) photo(s).`);
    doc.moveDown(0.2);
  };

  renderPhotoList('Extérieur', exterior);
  renderPhotoList('Intérieur', interior);
  renderPhotoList('Variantes couleur', colors);
}
...
function renderMaintenanceVd(doc, block) {
  ...
  const d = (block.data && block.data.data) ? block.data.data : (block.data || {});
  const intervals = Array.isArray(d.maintenance) ? d.maintenance : [];

  if (intervals.length === 0) {
    subMuted(doc, 'Programme d\'entretien non disponible pour ce VIN (couverture US uniquement).');
    return;
  }

  subMuted(doc, 'Programme d\'entretien constructeur. Permet de vérifier si les révisions programmées ont pu être effectuées.');

  intervals.slice(0, 15).forEach((interval) => {
    needPage(doc, 40);
    const mi = interval.mileage;
    const km = mi && mi.km ? String(mi.km) + ' km' : '';
    const miles = mi && mi.miles ? '(' + String(mi.miles) + ' mi)' : '';
    const label = [km, miles].filter(Boolean).join(' ') || 'Intervalle inconnu';

    doc.font('Helvetica-Bold').fontSize(9.5).fillColor(COL.accent);
    doc.text('▸ À ' + label, 50, doc.y);
    doc.moveDown(0.2);

    const services = Array.isArray(interval.service_items) ? interval.service_items : [];
    if (services.length > 0) {
      services.forEach((s) => {
        needPage(doc, 14);
        doc.font('Helvetica').fontSize(8.5).fillColor(COL.ink);
        doc.text('  • ' + String(s), 50, doc.y, { width: doc.page.width - 100 });
        doc.moveDown(0.1);
      });
    } else {
      doc.font('Helvetica').fontSize(8.5).fillColor(COL.muted);
      doc.text('  (détails non spécifiés)', 50, doc.y);
      doc.moveDown(0.1);
    }
    doc.moveDown(0.3);
  });

  if (intervals.length > 15) subMuted(doc, `… et ${intervals.length - 15} autre(s) intervalle(s) non affichés.`);
}
...
function renderWarrantyVd(doc, block) {
  if (!block || block.ok === false || block.error || block.skipped) {
    blockPreamble(doc, block);
    if (block && block.skipped) subMuted(doc, 'La garantie nécessite marque/modèle/année — non disponibles pour ce véhicule.');
    return;
  }
  const d = (block.data && block.data.data) ? block.data.data : (block.data || {});
  const warranty = d.warranty || {};

  if (!warranty || Object.keys(warranty).length === 0) {
    subMuted(doc, 'Données de garantie non disponibles pour ce véhicule.');
    return;
  }

  subMuted(doc, 'Garanties constructeur d\'origine (US/Canada 1992-2026). Vérifiez auprès du constructeur si la garantie est encore active.');
  ...
  Object.entries(warrantyTypes).forEach(([key, label]) => {
    ...
    const unlimited = String(w.unlimited || '').toLowerCase() === 'true' ? 'illimitée' : '';
