...
/**
 * Ancien format : { success, data: { make, model, year, ... } }.
 * Format actuel (OpenAPI) : { vin, specifications: { make, model, fuel, ... }, features }.
...
function extractVinDecodeIdentity(body) {
  ...
  if (body.specifications && typeof body.specifications === 'object') {
    ...
    return {
      ...
      year: yearNum,
      trim: '',
      engine: s.enginePower != null ? String(s.enginePower) : '',
...
async function fetchCarApiFullEnrichment(token, opts) {
  ...
  const identity = extractVinDecodeIdentity(dec.body);
