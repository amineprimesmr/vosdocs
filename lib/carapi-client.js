 ...
 * @param {string} token
 * @param {{ vin: string, country?: string, listingLimit?: number, listingOffset?: number, payment?: { price?: number, downPayment?: number, loanTerm?: number, interestRate?: number }, identityFallback?: { make?: string, model?: string, year?: string|number, trim?: string, engine?: string, transmission?: string, fuel_type?: string, drivetrain?: string } }} opts
 */
async function fetchCarApiFullEnrichment(token, opts) {
  ...
  const fb = opts.identityFallback && typeof opts.identityFallback === 'object' ? opts.identityFallback : null;
  if (fb) {
    const yrRaw = fb.year;
    let yr = typeof yrRaw === 'number' && Number.isFinite(yrRaw) ? yrRaw : parseInt(String(yrRaw || ''), 10);
    if (!Number.isFinite(yr)) yr = null;
    const mkFb = fb.make != null ? String(fb.make).trim() : '';
    const mdFb = fb.model != null ? String(fb.model).trim() : '';
    if (!identity) {
      if (mkFb && mdFb && yr != null) {
        identity = {
          make: mkFb,
          model: mdFb,
          year: yr,
          trim: fb.trim != null ? String(fb.trim) : '',
          engine: fb.engine != null ? String(fb.engine) : '',
          transmission: fb.transmission != null ? String(fb.transmission) : '',
          fuel_type: fb.fuel_type != null ? String(fb.fuel_type) : '',
          drivetrain: fb.drivetrain != null ? String(fb.drivetrain) : '',
          fromIdentityFallback: true
        };
      }
    } else {
      if (!String(identity.make || '').trim() && mkFb) identity.make = mkFb;
      if (!String(identity.model || '').trim() && mdFb) identity.model = mdFb;
      if ((!identity.year || !Number.isFinite(identity.year)) && yr != null) identity.year = yr;
      if ((!String(identity.engine || '').trim() && fb.engine) ||
        (!String(identity.fuel_type || '').trim() && fb.fuel_type) ||
        (!String(identity.transmission || '').trim() && fb.transmission) ||
        (!String(identity.drivetrain || '').trim() && fb.drivetrain)) {
        identity.fromIdentityFallback = true;
      }
      if (!String(identity.engine || '').trim() && fb.engine) identity.engine = String(fb.engine);
      if (!String(identity.fuel_type || '').trim() && fb.fuel_type) identity.fuel_type = String(fb.fuel_type);
      if (!String(identity.transmission || '').trim() && fb.transmission) identity.transmission = String(fb.transmission);
      if (!String(identity.drivetrain || '').trim() && fb.drivetrain) identity.drivetrain = String(fb.drivetrain);
    }
  }

  const decodeHttpOk = !!(dec.ok && (dec.status === 200 || dec.status === 201));
  const decodeOk = !!identity && (decodeHttpOk || !!(fb && fb.make && fb.model));
  ...
  const out = {
    ...
    decode: {
      ...
      carapiVinDecodeHttpOk: decodeHttpOk
    }
  };
  ...
  out.enrichment = {
    ...
    /** Documenté côté CarAPI (ex. STK) — requêtes d’inspection. */
    inspectionQueryCountry: inspectionCountry,
    listingLimit,
    listingOffset,
    /** Paramètres passés à GET /payments/{vin} (même logique que le playground). */
    payment: {
      price,
      downPayment,
      loanTerm,
