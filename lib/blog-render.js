...
function headBlock(config, title, description, canonical, isArticle, article, basePath = '../') {
  ...
  if (isArticle && article) {
    ...
    {"@context":"https://schema.org","@type":"Article","headline":${JSON.stringify(article.title)},"description":${JSON.stringify(description)},"datePublished":"${dateStr}","author":{"@type":"Organization","name":"VosDocs"},"publisher":{"@type":"Organization","name":"VosDocs","url":"${baseUrl}"}}
    </script>`;
  }
  return `
    <meta charset="UTF-8">
...
function renderBlogIndex(config, posts) {
  ...
  const title = config.seo?.indexTitle || (config.title || 'Blog') + ' | VosDocs';
...
<body>
  ...
  <header class="header header-light blog-header">
    <div class="container header-inner">
      <a href="${basePath}index.html" class="logo"><img src="/logo.png" alt="VosDocs"></a>
...
function renderBlogArticle(config, post, allPosts) {
  ...
  const basePath = '../../';
  const categories = config.categories || {};
  const title = post.title + ' | Blog VosDocs';
    ...
    <article class="blog-article">
      <div class="container container-narrow">
        ...
        <div class="blog-article-cta">
          ...
          <a href="${basePath}index.html" class="btn btn-primary btn-lg">Obtenir mon certificat — 19,90 €</a>
        </div>
        ${relatedHtml}
      </div>
    </article>
...
function renderBlogPublished(config) {
  const basePath = '../../';
  const blogPath = config.blogPath || '/blog';
  const title = 'Article publié | Blog VosDocs';
  const description = 'Votre article a été publié sur le blog VosDocs.';

  return `<!DOCTYPE html>
<html lang="fr">
<head>
${headBlock(config, title, description, (config.baseUrl || 'https://www.vosdocs.fr') + blogPath + '/published', false, null, basePath)}
</head>
<body>
