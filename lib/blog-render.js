...
function headBlock(config, title, description, canonical, isArticle, article, basePath = '../') {
  ...
  const brand = config.brandName || 'Carvingard';
  ...
  if (isArticle && article) {
    ...
    {"@context":"https://schema.org","@type":"Article","headline":${JSON.stringify(article.title)},"description":${JSON.stringify(description)},"datePublished":"${dateStr}","author":{"@type":"Organization","name":${JSON.stringify(brand)}},"publisher":{"@type":"Organization","name":${JSON.stringify(brand)},"url":"${baseUrl}"}}
    </script>`;
  }
  return `
    <meta charset="UTF-8">
...
const footerHtml = (basePath, config) => {
  const brand = escapeHtml((config && config.brandName) || 'Carvingard');
...
function renderBlogIndex(config, posts) {
  ...
  const brand = config.brandName || 'Carvingard';
  const title = config.seo?.indexTitle || (config.title || 'Blog') + ' | ' + brand;
...
<body>
  ...
  <header class="header header-light blog-header">
    <div class="container header-inner">
      <a href="${basePath}index.html" class="logo"><img src="/newlogo.png" alt="CarVINGuard"></a>
    ...
    <article class="blog-article">
      <div class="container container-narrow">
        ...
        <div class="blog-article-cta">
          ...
          <a href="${basePath}checkout.html" class="btn btn-primary btn-lg">Commander ici</a>
        </div>
        ${relatedHtml}
      </div>
    </article>
...
function renderBlogPublished(config) {
  ...
  const blogPath = config.blogPath || '/blog';
  const brand = config.brandName || 'Carvingard';
  const title = 'Article publié | Blog ' + brand;
  const description = 'Votre article a été publié sur le blog ' + brand + '.';

  return `<!DOCTYPE html>
<html lang="fr">
<head>
${headBlock(config, title, description, (config.baseUrl || 'https://www.carvingard.fr') + blogPath + '/published', false, null, basePath)}
</head>
<body>
